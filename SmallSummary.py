...
class ChunkSummaries(SmallSummary):
    def getSummeries(self, chunks: list, llm: object, vectorStore: object, workaround: str, framework: str) -> list[str]:
        ...
        i = 0
        display(f'{i}/{total_chunks} chunks summarized...')
        for chunk in chunks:
            ...
            display(f'{i}/{total_chunks} chunks summarized completed... ({round(end_time-start_time, 2)} secs)')

        interim_op_buffer.close()
        master_end_time = time.time()
        print(f'==== END OF SUMMARIZING CHUNKS {round(master_end_time-master_start_time, 2)} secs=====\n')
        return interim_op
